// for all countries name with ISO code
let country_list = [
  { name: "USA", code: "US" },
  { name: "Spain", code: "ES" },
  { name: "Italy", code: "IT" },
...
//for the change button setting
const change_country_btn = document.querySelector(".change-country-btn");
const close_list_btn = document.querySelector(".close-btn");
const input = document.getElementById("search-input");

// for creating countries list part
function createCountryList() {
  ...
  country_list.forEach((country, index) => {
    ...
    document.getElementById(
      `${ul_list_id}`
    ).innerHTML += `<li onclick=fetchData('${country.name}') id="${country.name}" class
    ="${country.code}">${country.name}</li`;
  });
}

let num_of_ul_list = 4;
createCountryList();

// toggling the show and hide the country list on click event.
...
// for the input setting and country filter
...
//for the reset all country on the list
